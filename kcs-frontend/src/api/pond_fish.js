...
export const checkParam = async (pondId)=>{
      try {
            const token = localStorage.getItem('token')
            if (!token) {
                  throw new Error('Token not find')
            }
            const res = await axios.get(`${REST_API_BASE_URL}/water_parameter/check/${pondId}`,{
                  headers: {
                        Authorization: `Bearer ${token}`
                  }
            })
...
export const updateWaterParam = async (pondId,data) => {
      try {
            const token = localStorage.getItem('token')
            if (!token) {
                  throw new Error('Token not find')
            }
            await axios.put(`${REST_API_BASE_URL}/water_parameter/update/${pondId}`,data, {
                  headers: {
                        'Authorization': `Bearer ${token}`
                  }
            })
...
export const getWaterParamHistory = async (pondid) => {
      try {
            const token = localStorage.getItem('token')
            if (!token) {
                  throw new Error('Token not find')
            }
            const res =await axios.get(`${REST_API_BASE_URL}/water_parameter/history/${pondid}`, {
                  headers: {
                        'Authorization': `Bearer ${token}`
                  }
            })
...
export const addFishToPond = async(pondId,fishId) =>{
      try {
            ...
            const res =  await axios.put(REST_API_BASE_URL +`/addFishToPond/pond/${pondId}/fish/${fishId}`, {},{
                  headers: {
                         Authorization: `Bearer ${token}`
                  }
            })
...
export const addFishHistory = async(fishId,data) =>{
      try {
            const token = localStorage.getItem('token')
            if (!token) {
                  throw new Error('Token not find')
            }
             await axios.post(REST_API_BASE_URL_FISH +`/${fishId}/addFishDevelopment`,data,{
                  headers: {
                         Authorization: `Bearer ${token}`
                  }
            })
...
export const getFishHistory = async (fishId) => {
      try {
            const token = localStorage.getItem('token')
            if (!token) {
                  throw new Error('Token not find')
            }
            const res = await axios.get(REST_API_BASE_URL_FISH + `/${fishId}/getFishDevelopment`, {
                  headers: {
                        Authorization: `Bearer ${token}`
                  }
            })
