...
import { deleteUser, getAllUser, setStatusUser } from "../../api/userService";
...
function UserInfo() {
  ...
  const handleSetRole = async () => {
    ...
    if(!roleToSet){
      toast.error("Please select a role to set.");
      return;
    }
    try {

      await Promise.all(selectedUsers.map((userId) => setRol(userId)));
      setUsers(users.filter((user) => !selectedUsers.includes(user.userId)));
      setSelectedUsers([]);
      
      toast.success(`Role set to ${roleToSet} for user ID: ${users.userId}`, {
        position: "top-right",
        autoClose: 800,
        hideProgressBar: true,
        closeOnClick: true,
      ...
      <Box sx={{ p: 3 }}>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Button
            ...
            color="secondary"
            onClick={() =>
              selectedUsers.forEach((userId) => handleSetRole(userId))
            }
            disabled={selectedUsers.length === 0}
          >
            Set Role
