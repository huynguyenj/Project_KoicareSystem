...
import { Edit, Delete, Search as SearchIcon, Save, ArrowBack } from "@mui/icons-material";
import {
  deleteProduct,
  getAllProductInShop,
  updateProduct,
...
const ShowProducts = () => {
  ...
  const handleImageChange = (e) => {
    ...
    setEditedData({ ...editedData, image: file }); // Update the image property with the selected file
    const reader = new FileReader();
    reader.onload = () => {
      setImageReview(reader.result); // Update the image preview state with the file contents
    };
  ...
  const handleSave = async (productId) => {
    try {
      ...
      console.log(formData.get("catogory"));
  ...
  const handleDelete = async (productId) => {
    try {
      console.log(editedData);
      const checked = window.confirm("Bạn có chắc bạn muốn xóa!");
      if (!checked) {
        return;
      }
  ...
  return (
    <>
      <TableContainer component={Paper} sx={{ mt: 5 }}>
        ...
        <Table sx={{ maxWidth: 1000, margin:'auto' }} aria-label="products table">
          <TableHead
            sx={{
