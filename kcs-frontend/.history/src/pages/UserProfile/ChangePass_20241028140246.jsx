...
function ChangePass() {
      const [oldPass, setOldPass] = useRef({oldPassword:''});
    ...
    <div>
            <Box maxWidth={500} sx={{margin:'auto'}}>
                  <Card elevation={2} sx={{padding:3}}>
                    ...
                    { message ?  <Alert sx={{mt:2}} security='success'>{message}</Alert> :(<Alert sx={{mt:2}} severity="error">Mật khẩu không chính xác!.</Alert>)} 
                  </Card>
            </Box>
    </div>
  )
