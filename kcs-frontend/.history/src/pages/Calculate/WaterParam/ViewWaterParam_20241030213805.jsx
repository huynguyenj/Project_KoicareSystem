...
function ViewWaterParam() {
  ...
  const recommendProduct = (key) => {
    if (recomendation) {
      ...
      let newProduct = {};
      if (key === "O2") {
        newProduct = { O2: productsList.filter((p) => p.category === "RESOLVE_O2") };
      } else if (key === "Nhiệt độ") {
        newProduct = { temp: productsList.filter((p) => p.category === "RESOLVE_TEMPERATURE") };
      } else if (key === "Độ mặn") {
        newProduct = { salt: productsList.filter((p) => p.category === "RESOLVE_SALINITY") };
      } else if (key === "pH") {
        newProduct = { pH: productsList.filter((p) => p.category === "RESOLVE_PH") };
      } else if (key === "NO2") {
        newProduct = { NO2: productsList.filter((p) => p.category === "RESOLVE_NO2") };
      } else if (key === "NO3") {
        newProduct = { NO3: productsList.filter((p) => p.category === "RESOLVE_NO3") };
      } else if (key === "PO4") {
        newProduct = { PO4: productsList.filter((p) => p.category === "RESOLVE_PO4") };
      }
      
      setRecomendProduct((prevRecomendProduct) => ({
        ...prevRecomendProduct,
        ...newProduct, // only update the clicked key
      }));
      setShow(true);
    }
  };
