      ...
      if(recomendation){
       ...
       setRecomendProduct((prev) => {
        const updatedRecomendProduct = { ...prev };
          ...
          if(key =="O2"){
            setRecomendProduct({O2 : [productsList.filter(p => p.category == "RESOLVE_O2")]})
            console.log(recomendProduct.O2)
          }if(key == "Nhiệt độ"){
            setRecomendProduct({temp : productsList.filter(p => p.category == "RESOLVE_TEMPERATURE")})
            console.log(recomendProduct.temp)
          }if(key == "Độ mặn"){
            setRecomendProduct({salt : productsList.filter(p => p.category == "RESOLVE_SALINITY")})
            console.log(recomendProduct.salt)
          }
          if(key == "pH"){
            setRecomendProduct({pH : productsList.filter(p => p.category == "RESOLVE_PH")})
            console.log(recomendProduct.pH)
          }
          if(key == "NO2"){
            setRecomendProduct({NO2 : productsList.filter(p => p.category == "RESOLVE_NO2")})
            console.log(recomendProduct.NO2)
          }
          if(key == "NO3"){
            setRecomendProduct({NO3 : productsList.filter(p => p.category == "RESOLVE_NO3")})
            console.log(recomendProduct.NO3)
          }
          if(key == "PO4"){
            setRecomendProduct({PO4 : productsList.filter(p => p.category == "RESOLVE_PO4")})
