                ...
                <>
                  ...
                  {!recomendation || Object.keys(recomendation).length === 0 ? (
                    ...
                    <Alert variant="filled" severity="warning" sx={{ mt: 2 }}>
                      ...
                      {show ? (
                        <>
                        <Typography variant="h6" sx={{mt:2}}>Một số sản phẩm: </Typography>
                          {Object.entries(recomendProduct).map(
                            ([category, products]) => (
                              <div key={category}>
                                <Typography>{categoryNames[category]}</Typography>
                                <ul>
                                  {products.slice(0, 3).map((product) => (
                                    <>
                                     <li key={product.id}>{product.productName}</li>
                                     <LinkIcon onClick={()=>naviagtor("/")}>Nhấp vào link để xem chi tiết sản phẩm</LinkIcon>
                                    </>
                                   
                                  ))}
                                  {products.length > 3 && <li>...</li>}
                                </ul>
                              </div>
                            )
                          )}
                          <Button color="white" onClick={()=>setShow(false)}><ArrowBackIcon/></Button>
                        </>
                      ) : (
                        <Button
                          variant="contained"
