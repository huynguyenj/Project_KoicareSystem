              ...
              {!update ? (
                <>
                  ...
                  {!recomendation || Object.keys(recomendation).length === 0 ? (
                    ...
                    {show ? (
  <>
    <Typography variant="h5" sx={{ mt: 2, mb: 2, textAlign: "center", color: "primary.main" }}>
      Đề xuất 1 số sản phẩm cải thiện thông số: 
    </Typography>
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      {Object.entries(recomendProduct).map(([category, products]) => (
        <Card key={category} sx={{ p: 2, bgcolor: "background.paper", boxShadow: 3 }}>
          ...
          <List dense>
            {products.slice(0, 3).map((product) => (
              <ListItem key={product.id} sx={{ display: "flex", alignItems: "center" }}>
                <Typography variant="body1" sx={{ flexGrow: 1 }}>{product.productName}</Typography>
                <IconButton color="primary" onClick={() => naviagtor("/")}>
                  <LinkIcon />
                </IconButton>
              </ListItem>
            ))}
