...
function ViewWaterParam() {
  ...
  const recommendProduct = (key) => {
    if (recomendation) {
      let newProducts = {};
      if (key == "O2") {
        newProducts = { O2: productsList.filter(p => p.category === "RESOLVE_O2") };
      } else if (key == "Nhiệt độ") {
        newProducts = { temp: productsList.filter(p => p.category === "RESOLVE_TEMPERATURE") };
      } else if (key == "Độ mặn") {
        newProducts = { salt: productsList.filter(p => p.category === "RESOLVE_SALINITY") };
      } else if (key == "pH") {
        newProducts = { pH: productsList.filter(p => p.category === "RESOLVE_PH") };
      } else if (key == "NO2") {
        newProducts = { NO2: productsList.filter(p => p.category === "RESOLVE_NO2") };
      } else if (key == "NO3") {
        newProducts = { NO3: productsList.filter(p => p.category === "RESOLVE_NO3") };
      } else if (key == "PO4") {
        newProducts = { PO4: productsList.filter(p => p.category === "RESOLVE_PO4") };
      } 
      // Update the state using a callback
      setRecomendProduct(prevState => ({ ...prevState, ...newProducts }));
      co
    }
  };
