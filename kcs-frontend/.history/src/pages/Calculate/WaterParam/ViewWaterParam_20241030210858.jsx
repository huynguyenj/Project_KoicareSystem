...
function ViewWaterParam() {
  ...
  const recommendProduct = (key) => {
    if (recomendation) {
    
      setRecomendProduct((prev) => {
        const updatedRecomendProduct = { ...prev };
  
        if (key === "O2") {
          updatedRecomendProduct.O2 = productsList.filter(p => p.category === "RESOLVE_O2");
          console.log(updatedRecomendProduct.O2);
        }
        if (key === "Nhiệt độ") {
          updatedRecomendProduct.temp = productsList.filter(p => p.category === "RESOLVE_TEMPERATURE");
          console.log(updatedRecomendProduct.temp);
        }
        if (key === "Độ mặn") {
          updatedRecomendProduct.salt = productsList.filter(p => p.category === "RESOLVE_SALINITY");
          console.log(updatedRecomendProduct.salt);
        }
        if (key === "pH") {
          updatedRecomendProduct.pH = productsList.filter(p => p.category === "RESOLVE_PH");
          console.log(updatedRecomendProduct.pH);
        }
        if (key === "NO2") {
          updatedRecomendProduct.NO2 = productsList.filter(p => p.category === "RESOLVE_NO2");
          console.log(updatedRecomendProduct.NO2);
        }
        if (key === "NO3") {
          updatedRecomendProduct.NO3 = productsList.filter(p => p.category === "RESOLVE_NO3");
          console.log(updatedRecomendProduct.NO3);
        }
        if (key === "PO4") {
          updatedRecomendProduct.PO4 = productsList.filter(p => p.category === "RESOLVE_PO4");
          console.log(updatedRecomendProduct.PO4);
        }
        
        console.log()
        return updatedRecomendProduct;
      });
    }
  };
                    ...
                    <Alert variant="filled" severity="warning" sx={{ mt: 2 }}>
                      {Object.entries(recomendation).map(([key, value]) => (
                        <>
                          ...
                          {show? 
                          ""
                          :<><Button color="inherit" onClick={()=>recommendProduct(key)}><Info/></Button></>}
