  ...
  const recommendProduct = (key) =>{
      if(recomendation){
       console.log(recomendation)
          if(key =="O2"){
            setRecomendProduct({...recomendProduct, O2 : [productsList.filter(p => p.category == "RESOLVE_O2")]})
            console.log(recomendProduct.O2)
          }if(key == "Nhiệt độ"){
            setRecomendProduct({...recomendProduct, temp : productsList.filter(p => p.category == "RESOLVE_TEMPERATURE")})
            console.log(recomendProduct.temp)
          }if(key == "Độ mặn"){
            setRecomendProduct({...recomendProduct, salt : productsList.filter(p => p.category == "RESOLVE_SALINITY")})
            console.log(recomendProduct.salt)
          }
          if(key == "pH"){
            setRecomendProduct({...recomendProduct, pH : productsList.filter(p => p.category == "RESOLVE_PH")})
            console.log(recomendProduct.pH)
          }
          if(key == "NO2"){
            setRecomendProduct({...recomendProduct, NO2 : productsList.filter(p => p.category == "RESOLVE_NO2")})
            console.log(recomendProduct.NO2)
          }
          if(key == "NO3"){
            setRecomendProduct({...recomendProduct, NO3 : productsList.filter(p => p.category == "RESOLVE_NO3")})
            console.log(recomendProduct.NO3)
          }
          if(key == "PO4"){
            setRecomendProduct({...recomendProduct, PO4 : productsList.filter(p => p.category == "RESOLVE_PO4")})
            console.log(recomendProduct.PO4)
          }
                    ...
                    <Alert variant="filled" severity="warning" sx={{ mt: 2 }}>
                      {Object.entries(recomendation).map(([key, value]) => (
                        <>
                          ...
                          {show? (recomendProduct.map(p=>{})):<><Button color="inherit" onClick={()=>recommendProduct(key)}><Info/></Button></>}
