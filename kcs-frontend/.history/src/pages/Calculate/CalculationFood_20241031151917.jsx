...
function CalculationFood() {
  ...
  const calculateFeedingAmount = (weight, age, date) => {
    let feedingPercentage = isSummerSeason(date) ? 0.03 : 0.02;
  
    // Adjust feeding percentage based on age group
    if (age < 1) {          // Less than 1 year old
      feedingPercentage += 0.01;  // Higher rate for young fish
    } else if (age >= 1 && age <= 3) { // 1-3 years
      feedingPercentage += 0.005;
    } // No additional percentage for older fish
  ...
  return (
    <Box
      ...
      alignItems="center"
      sx={{ maxWidth: '600px', margin: 'auto', padding: 3,mt:10 }}
    >
      <Paper elevation={3} sx={{ padding: 3, width: '100%' }}>
        ...
        {selectedFish.fishId && (
          <Box mt={1} p={2} bgcolor="lightBlue" borderRadius={2}>
            ...
            <Typography>Tuổi: {selectedFish.fishName}</Typography>
            <Typography>
              Tỉ lệ thức ăn: {isSummerSeason(date) ? '3%' : '2%'}
            </Typography>
