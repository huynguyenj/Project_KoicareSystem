...
if (!listBlogs) {
  ...
  const search = listBlogs.filter(
        (blog) =>
        blog.title.toLowerCase().includes(searchQuery.toLowerCase())||
        blog.content.toLowerCase().includes(searchQuery.toLowerCase())||
        blog.userName.toLowerCase().includes(query.username)
      )
  ...
  const sort = (blogs)=>{
    return [...blogs].sort((a,b) => {
      ...
      :new Date(a.publishedDate) - new Date(a.publishedDate)
    })
  ...
  const toggleSortOrder = () => {
    ...
    setSortItem(sort(search(searchQuery)));
  };
  ...
  const handleSearchChange = (e) =>{
    ...
    if(query){
      const filteredBlogs = sort(search(query));
      setSortItem(filteredBlogs); 
    }
    const filter = search(query)
    setListBlogs(sort(filter))
  }
