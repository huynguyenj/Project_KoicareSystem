...
import { deleteOrderFail, order } from "../../api/product";
...
function PaymentSuccess() {
  ...
  const verifyPayments = async () => {
    try {
      ...
      const orderItems = orderDetail.order.map(orderI => ({
        productId: orderI.productId,
        shopId: orderI.shopId,
     ...
     const formData = {
      order: orderItems,
      orderId: orderIdInfo ? [orderIdInfo] : [] // Ensure orderId is an array
    };
      console.log(formData)
      setDeleteInfo(formData)
      ...
      if (res.code === 0) {
        ...
        await deleteOrderFail(deleteInfo)
