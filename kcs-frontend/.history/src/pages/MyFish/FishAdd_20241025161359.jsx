...
const FishForm = () => {
  ...
  const validateFields = () => {
    const newErrors = {};
    if (!formData.name || fishData.name.trim() === "") {
      newErrors.name = "Tên ao không được để trống";
    }
    if (!formData.age || fishData.age <= 0) {
      newErrors.age = "Tuổi phải lớn hơn 0";
    }
    if (!formData.price || fishData.price <= 0) {
      newErrors.price = "Giá trị phải lớn hơn 0";
    }
    if (!formData.size || fishData.size <= 0) {
      newErrors.size = "Kích thước phải lớn hơn 0";
    }
    if (!formData.weight || fishData.weight <= 0) {
      newErrors.weight = "Cân nặng phải lớn hơn 0";
    }
    if (!formData.shape || fishData.shape.trim() === "") {
      newErrors.shape = "Vóc dáng không được để trống";
    }
    if (
      !formData.gender ||
      !(formData.gender.trim().toLowerCase() === "đực" || fishData.gender.trim().toLowerCase() === "cái")
    ) {
      newErrors.gender = "Giới tính phải là đực hoặc cái";
    }
    if (!formData.type || fishData.type.trim() === "") {
      newErrors.type = "Giống loài không được để trống";
    }
    if (!formData.origin || fishData.origin.trim() === "") {
      newErrors.origin = "Xuất xứ không được để trống";
    }
    if (!formData.health || fishData.health.trim() === "") {
