...
const FishForm = () => {
  ...
  const validateFields = () => {
    const newErrors = {};
    if (!formData.name || fỏmData.name.trim() === "") {
      newErrors.name = "Tên ao không được để trống";
    }
    if (!formData.age || fỏmData.age <= 0) {
      newErrors.age = "Tuổi phải lớn hơn 0";
    }
    if (!formData.price || fỏmData.price <= 0) {
      newErrors.price = "Giá trị phải lớn hơn 0";
    }
    if (!formData.size || fỏmData.size <= 0) {
      newErrors.size = "Kích thước phải lớn hơn 0";
    }
    if (!formData.weight || fỏmData.weight <= 0) {
      newErrors.weight = "Cân nặng phải lớn hơn 0";
    }
    if (!formData.shape || fỏmData.shape.trim() === "") {
      newErrors.shape = "Vóc dáng không được để trống";
    }
    if (
      !formData.gender ||
      !(formData.gender.trim().toLowerCase() === "đực" || fỏmData.gender.trim().toLowerCase() === "cái")
    ) {
      newErrors.gender = "Giới tính phải là đực hoặc cái";
    }
    if (!formData.type || fỏmData.type.trim() === "") {
      newErrors.type = "Giống loài không được để trống";
    }
    if (!formData.origin || fỏmData.origin.trim() === "") {
      newErrors.origin = "Xuất xứ không được để trống";
    }
    if (!formData.health || fỏmData.health.trim() === "") {
