...
const FishForm = () => {
  ...
  const validateFields = () => {
    ...
    if (!formData.fishShape || formData.shape.trim() === "") {
      newErrors.shape = "Vóc dáng không được để trống";
    }
    if (
      !formData.gender ||
      !(formData.gender.trim().toLowerCase() === "đực" || formData.gender.trim().toLowerCase() === "cái")
    ) {
    ...
    if (!formData.health || formData.health.trim() === "") {
