              ...
              {cartItems.map((item) => (
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
  onClick={() => {
    if (item.quantity < item.product.stock) { // Check if current quantity is less than stock
      updateQuantity(item.product.id, item.quantity + 1);
    }
  }}
  disabled={item.quantity >= item.product.stock} // Disable button if at stock limit
>
  <AddIcon />
</IconButton>
                    edge="end"
